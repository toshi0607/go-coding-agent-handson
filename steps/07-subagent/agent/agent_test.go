...
import (
	...
	"github.com/toshi0607/go-coding-agent-handson/steps/07-subagent/contextmgr"
	"github.com/toshi0607/go-coding-agent-handson/steps/07-subagent/llm"
	"github.com/toshi0607/go-coding-agent-handson/steps/07-subagent/permission"
...
func TestTaskToolRunsSubAgent(t *testing.T) {
	...
	// コンテキスト分離の検証。サブエージェントへのリクエスト(2回目の
	// LLM呼び出し)に含まれるのは prompt の1メッセージだけで、
	// 親の会話履歴は一切渡っていないこと。
	subReq := client.Requests()[1]
	if len(subReq.Messages) != 1 {
		t.Fatalf("サブエージェントには prompt の1メッセージだけを渡すはず: %d件", len(subReq.Messages))
	}
	subFirst, _ := json.Marshal(subReq.Messages[0])
	if !strings.Contains(string(subFirst), "認証処理の場所を調べて") {
		t.Errorf("サブエージェントに prompt が渡っていない: %s", subFirst)
	}
	if strings.Contains(string(subFirst), "認証処理どこ?") {
		t.Errorf("親のユーザー入力がサブエージェントに漏れている: %s", subFirst)
	}
}
