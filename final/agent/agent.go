...
func (a *Agent) Run(ctx context.Context, userInput string) (string, error) {
	...
	for range a.maxIterations {
		resp, err := a.client.Stream(ctx, a.buildParams(), func(text string) {
...
//  1. PreToolUseフック  — ユーザー定義の検査で、承認より先に機械的に弾く
//  2. 権限チェック      — 承認フロー。ユーザーの最終判断
//  3. ツール実行
//  4. 結果の切り詰め    — コンテキスト保護
//  5. PostToolUseフック — ログ記録など
//
...
func (a *Agent) executeTool(ctx context.Context, block anthropic.ContentBlockUnion) anthropic.ContentBlockParamUnion {
	...
	if err := a.hooks.RunPre(ctx, block.Name, block.Input); err != nil {
		fmt.Fprintf(a.out, "  ⨯ %v\n", err)
